    ...
    return {
        ...
        getImportsPixelFormatsYear: function () {
            ...
            var importsArray = db.getCollection('prod').aggregate([
                ...
                { $unwind: '$probe.streams' },
                { $match: {'probe.streams.codec_type': 'video'} },
                { $group : { _id :  { year: "$year", month: "$month", month_num:"$month_num", pixel_format : { $ifNull: [ "$probe.streams.pix_fmt", "UNKNOWN" ] }, codec_long_name:{ $toString:"$probe.streams.codec_long_name" } }, total: { $sum : 1 } }},
                { $group : { _id :  "$_id.year", months: { $push: { month:"$_id.month", month_num:"$_id.month_num", pixel_format : "$_id.pixel_format", codec_long_name : "$_id.codec_long_name", total : "$total" }}}},
                {$unwind:"$months"},
                {$project: {year:"$_id", month:"$months.month", month_num:"$months.month_num", pixel_format : "$months.pixel_format", codec_long_name : "$months.codec_long_name", total:"$months.total"}}, 
                  { $sort : { year : 1, month_num: 1}},
             ]).toArray(); 
            //printjson(importsArray);
            return importsArray            
        },
        getImportsProfilesYear: function () {
            print('\nImports\n==============================');
            var importsArray = db.getCollection('prod').aggregate([
                { $project : { probe: 1, created: { $ifNull: [ "$created", new Date("2000-01-01T00:00:00Z") ] } } },
                { $project : { probe: 1, month_num : {$month : { $toDate: "$created"  }}, year : {$year :  { $toDate: "$created"  }},}},
                ...
                { $unwind: '$probe.streams' },
                { $match: {'probe.streams.codec_type': 'video'} },
                { $group : { _id :  { year: "$year", month: "$month", month_num:"$month_num", profile : { $ifNull: [ "$probe.streams.profile", "UNKNOWN" ] }, codec_long_name:{ $toString:"$probe.streams.codec_long_name" } }, total: { $sum : 1 } }},
                { $group : { _id :  "$_id.year", months: { $push: { month:"$_id.month", month_num:"$_id.month_num", profile : "$_id.profile", codec_long_name : "$_id.codec_long_name", total : "$total" }}}},
                {$unwind:"$months"},
                {$project: {year:"$_id", month:"$months.month", month_num:"$months.month_num", profile : "$months.profile", codec_long_name : "$months.codec_long_name", total:"$months.total"}}, 
                  { $sort : { year : 1, month_num: 1}},
             ]).toArray(); 
            //printjson(importsArray);
            return importsArray            
        },
        getImportsFrameRatesYear: function () {
            print('\nImports\n==============================');
            var importsArray = db.getCollection('prod').aggregate([
                { $project : { probe: 1, created: { $ifNull: [ "$created", new Date("2000-01-01T00:00:00Z") ] } } },
                { $project : { probe: 1, month_num : {$month : { $toDate: "$created"  }}, year : {$year :  { $toDate: "$created"  }},}},
                ...
                { $unwind: '$probe.streams' },
                { $match: {'probe.streams.codec_type': 'video'} },
                { $group : { _id :  { year: "$year", month: "$month", month_num:"$month_num", frame_rate : { $ifNull: [ "$probe.streams.r_frame_rate", "0" ] }, codec_long_name:{ $toString:"$probe.streams.codec_long_name" } }, total: { $sum : 1 } }},
                { $group : { _id :  "$_id.year", months: { $push: { month:"$_id.month", month_num:"$_id.month_num", frame_rate : "$_id.frame_rate", codec_long_name : "$_id.codec_long_name", total : "$total" }}}},
                {$unwind:"$months"},
                {$project: {year:"$_id", month:"$months.month", month_num:"$months.month_num", frame_rate : "$months.frame_rate", codec_long_name : "$months.codec_long_name", total:"$months.total"}}, 
                  { $sort : { year : 1, month_num: 1}},
            ]).toArray(); 
            //printjson(importsArray);
            return importsArray            
        },
        getImportsStatusYear: function () {
            print('\nImports\n==============================');
            var importsArray = db.getCollection('prod').aggregate([
                { $project : { probe: 1, status: 1, created: { $ifNull: [ "$created", new Date("2000-01-01T00:00:00Z") ] } } },
                { $project : { probe: 1, status: 1, month_num : {$month : { $toDate: "$created"  }}, year : {$year :  { $toDate: "$created"  }},}},
                { $project : { 
                    month: {
                        $let: {
                            vars: {
                ...
                { $unwind: '$probe.streams' },
                { $group : { _id :  { year: "$year", month: "$month", month_num:"$month_num", status : "$status", codec_long_name : "$probe.streams.codec_long_name"}, total: { $sum : 1 } }},
                { $group : { _id :  "$_id.year", months: { $push: { month:"$_id.month", month_num:"$_id.month_num", status : "$_id.status", codec_long_name : "$_id.codec_long_name", total : "$total" }}}},
                {$unwind:"$months"},
                {$project: {year:"$_id", month:"$months.month", month_num:"$months.month_num", status : "$months.status", codec_long_name : "$months.codec_long_name", total:"$months.total"}}, 
                  { $sort : { year : 1, month_num: 1}},
             ]).toArray(); 
        //printjson(importsArray);
        return importsArray            
